...
async function viewConsumption(view) {
  ...
  const load = async () => {
    ...
    // Daily: opening is "yesterday's closing stock" relative to From --
    // picking Sep 1 as From shows the balance as of Aug 31, matching the
    // standard "this period's opening = last period's closing"
    // relationship. Monthly: opening always shows the product's first-ever
    // receive quantity, fixed, no matter which From/To is picked. Closing
    // stock is always the balance as of the report's To date, for both.
    const openingLine = sumByUnit(d.totals, 'opening_stock', 'unit');
    const closingLine = sumByUnit(d.totals, 'closing_stock', 'unit');
    const openingFoot = granularity === 'month' ? 'first ever received' : `as of ${fmtDate(d.from)}`;

    summary.innerHTML = `
      <div class="card stat"><div class="label">Opening stock</div>
        <div class="value" style="font-size:16px">${esc(openingLine) || '0'}</div>
        <div class="foot">${esc(openingFoot)}</div></div>
    ...
    if (d.product) {
      ...
      const rowsQtyLine = sumByUnit(d.rows, 'qty', 'unit');
      body.innerHTML = `
        <div class="card" style="margin-bottom:16px"><div class="card-head">Totals by product <span style="font-weight:500;color:var(--muted)">— opening stock ${granularity === 'month' ? 'as first received' : `as of ${esc(fmtDate(d.from))}`}, closing as of ${esc(fmtDate(d.to))}</span></div>
          <div class="table-wrap"><table>
