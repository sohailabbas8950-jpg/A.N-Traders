...
route('GET', '/api/consumption', async (ctx) => {
  ...
  // Opening stock: the ledger balance at the start of each row's period, and
  // at the start of the whole report range for each product's totals --
  // computed from every movement of any kind (receive/issue/transfer/
  // adjust), not just the 'issue' movements this report otherwise counts,
  // and not bounded by the report's own `from` filter, since stock a
  // product already had before the report window (or before this app was
  // ever used to log anything) still needs to be included. If a business's
  // real starting stock was never entered as a movement, this reads 0 for
  // it -- there is no way to recover a quantity that was never recorded.
  // Logging it as a 'receive' (or 'adjust') dated at go-live will flow
  // through here automatically from then on.
  ...
  if (productIds.length) {
    ...
    for (const r of rows) {
      const periodBoundary = granularity === 'month' ? `${r.period}-01T00:00:00.000Z` : `${r.period}T00:00:00.000Z`;
      const periodEnd = nextPeriodBoundary(r.period);
      const list = byProduct.get(r.product_id) || [];
      let opening = 0;
      let closing = 0;
      for (const h of list) {
        if (h.ts < periodBoundary) opening += h.delta;
        if (h.ts < periodEnd) closing += h.delta; else break;
      }
