...
route('GET', '/api/consumption', async (ctx) => {
  ...
  // Opening stock (per-product totals): the ledger balance strictly before
  // the report's `from` date -- i.e. "yesterday's closing stock" relative
  // to whatever From is selected (From=Sep 1 -> opening is the balance as
  // of Aug 31; a single day selected as both From and To -> opening is
  // that same balance as of the end of the day before). This is the
  // standard "this period's opening = the previous period's closing"
  // relationship, computed from every movement of any kind (receive/
  // issue/transfer/adjust), not just the 'issue' movements this report
  // otherwise counts, and looking back through the FULL history regardless
  // of how far `from` reaches -- stock a product already had years before
  // the report window still needs to be included.
  // EXCEPTION: if the product has no movements at all before `from` (it
  // simply hadn't been received yet as of that date), the balance is
  // genuinely 0 -- but instead of surfacing a bare 0, opening falls back to
  // the quantity of that product's first-ever receive, so a report window
  // picked before a product's stock arrived still shows the real stock
  // that eventually came in rather than looking like empty/missing data.
  // This fallback only fires on true "didn't exist yet" zeros; a product
  // that had real history before `from` which happens to net to 0 (fully
  // received then fully issued before the window) keeps its real 0.
  ...
  if (productIds.length) {
    ...
    for (const pid of productIds) {
      ...
      let opening = 0;
      let closing = 0;
      let hasHistoryBeforeFrom = false;
      for (const h of list) {
        if (h.ts < fromBoundary) { opening += h.delta; hasHistoryBeforeFrom = true; }
        if (h.ts <= toBoundary) closing += h.delta;
      }
      // If the product had NO movements at all yet as of the From date (it
      // simply hadn't been received into these locations yet), the real
      // balance is genuinely 0 -- but showing a bare 0 there reads as "no
      // data" when really there's a first delivery waiting later in this
      // same list. In that specific case only, show the quantity of its
      // first-ever receive instead of 0, so the report still surfaces the
      // real stock that eventually showed up. This does NOT apply when the
      // product has real prior history that simply nets to 0 (e.g.
      // received and fully issued before `from`) -- that 0 is a true,
      // date-sensitive balance and is left alone.
      if (!hasHistoryBeforeFrom) {
        const firstReceive = list.find((h) => h.kind === 'receive');
        if (firstReceive) opening = firstReceive.delta;
      }
  ...
  const info = await db.run(
    `INSERT INTO movements
       (ts, kind, product_id, from_location_id, to_location_id, qty, entry_qty, entry_unit,
        batch_no, expiry, unit_cost, reference, party, note, user_id)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
