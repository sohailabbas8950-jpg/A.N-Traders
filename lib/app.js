...
route('GET', '/api/consumption', async (ctx) => {
  ...
  // Opening stock (per-product totals): always the quantity from that
  // product's very first 'receive' movement at one of the report's visible
  // locations -- a fixed reference point, deliberately NOT bounded by (or
  // sensitive to) the report's own `from` filter. Before this, opening
  // stock was "the ledger balance strictly before `from`", which reads as a
  // confusing 0 for any product whose real history only starts partway
  // through a wide default range (e.g. a product added this week, viewed
  // in a report going back 12 months) even though that 0 was technically
  // correct at that exact date. Picking any From/To no longer changes this
  // number -- it's always that product's actual starting quantity. If a
  // product has never had a 'receive' movement (e.g. it only ever arrived
  // via an adjustment), this reads 0 -- there's no receive to anchor to.
  // Closing stock is unrelated and still means "balance as of the report's
  // To date", which is exactly what a live "how much do we have now"
  // number needs to be.
  const productIds = [...new Set(totals.map((t) => t.product_id))];
  const openingByRowKey = new Map();
  const firstReceiveQty = new Map();
  ...
  if (productIds.length) {
    ...
    for (const pid of productIds) {
      ...
      // history is ordered by ts ascending, so the first 'receive' entry
      // found is chronologically the product's very first receipt. Its
      // delta equals its qty (a receive only ever has a to_location_id, no
      // from_location_id, so nothing is subtracted).
      const firstReceive = list.find((h) => h.kind === 'receive');
      firstReceiveQty.set(pid, firstReceive ? firstReceive.delta : 0);
    }
  ...
  for (const t of totals) {
    t.opening_stock = firstReceiveQty.get(t.product_id) || 0;
