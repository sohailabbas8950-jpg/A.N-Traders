...
route('POST', '/api/movements', async (ctx) => {
  ...
  const unitCost = num(body.unit_cost, product.cost_price);
  ...
  // A receive that takes effect immediately (admin/manager, or any
  // non-gated case) also becomes the product's new going-forward Cost
  // Price -- the rate this shipment actually came in at is the best known
  // current price until the next receive changes it again. This never
  // rewrites any OTHER movement's own already-recorded unit_cost, only the
  // product's default used for future issues/receives/reports. A still-
  // pending receive hasn't taken effect yet, so it waits for approval (see
  // the /approve route below) to do this.
  if (kind === 'receive' && !needsApproval) {
    await db.run('UPDATE products SET cost_price = ? WHERE id = ?', [Math.max(0, unitCost), product.id]);
  }
...
route('PUT', '/api/movements/:id', async (ctx) => {
  ...
  const unitCost = num(body.unit_cost, existing.unit_cost);
  ...
  // If this is a live (already-approved) receive, editing its rate also
  // resyncs the product's going-forward Cost Price to match it -- the same
  // rule that applies when a receive is created or approved. A pending
  // receive edited directly (bypassing the Review flow) hasn't taken
  // effect yet, so it's left alone here.
  if (kind === 'receive' && existing.status === 'approved') {
    await db.run('UPDATE products SET cost_price = ? WHERE id = ?', [Math.max(0, unitCost), product.id]);
  }
...
route('POST', '/api/movements/:id/approve', async (ctx) => {
  ...
  // Approving is when this receive actually takes effect, so this is when
  // its rate (whether admin corrected it or left it as submitted) becomes
  // the product's new going-forward Cost Price -- same rule as an
  // immediately-approved receive above.
  await db.run('UPDATE products SET cost_price = ? WHERE id = ?', [Math.max(0, unitCost), product.id]);
