...
route('GET', '/api/consumption', async (ctx) => {
  ...
  if (!locIds.length) return sendJson(ctx.res, 200, { rows: [], totals: [], movements: [], grand_total: { movements: 0 } });
  ...
  const rows = await db.all(
    `SELECT m.product_id, p.sku, p.name, p.unit, ${periodExpr} AS period,
            SUM(m.qty) AS qty, COUNT(*) AS movements
       FROM movements m
       JOIN products p ON p.id = m.product_id
  ...
  if (singleProductId) {
    movements = await db.all(
      `SELECT m.id, m.ts, m.qty, m.entry_qty, m.entry_unit, m.reference, m.party, m.note,
              fl.name AS from_name, u.name AS user_name
         FROM movements m
         LEFT JOIN locations fl ON fl.id = m.from_location_id
         LEFT JOIN users u ON u.id = m.user_id
        WHERE m.kind = 'issue' AND m.from_location_id IN (${ph})
  ...
  sendJson(ctx.res, 200, {
    ...
    totals,
    movements,
    grand_total: { movements: grandMovements },
  });
});
