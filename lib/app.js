...
route('GET', '/api/consumption', async (ctx) => {
  ...
  if (!locIds.length) return sendJson(ctx.res, 200, { rows: [], totals: [], grand_total: { value: 0, movements: 0 } });
  ...
  const totals = [...totalsMap.values()].sort((a, b) => b.value - a.value);
  ...
  sendJson(ctx.res, 200, {
    ...
    rows,
    totals,
    grand_total: { value: grandValue, movements: grandMovements },
  });
});
